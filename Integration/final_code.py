...
# --- PIPELINE 2 IMPORTS (from step2_calibration.py) ---
import keyboard
import serial
import random
import winsound
...
# --- PIPELINE 1 CLASSES (Unchanged Logic) ---
...
# --- PIPELINE 2 CLASS (Logic Preserved) ---
class ChipCalibrationManager:
    """ 
    Adapted from HandsToolV4 in step2_calibration.py
    Manages Chip Coordinates and Arduino Clicking
    """
    def __init__(self, parent_frame, root_window):
        self.frame = parent_frame
        self.root_window = root_window # We need the main window to hide/show it
        ...
        # --- UI HEADER (Adapted to Frame) ---
        tk.Label(self.frame, text="Step 2: Chip Selector", font=("Arial", 16, "bold")).pack(pady=10)
        tk.Label(self.frame, text="Window hides automatically during MAP and TEST", fg="blue").pack()
        
        self.lbl_hw = tk.Label(self.frame, text="Arduino: Connecting...", fg="orange", font=("Arial", 10))
        self.lbl_hw.pack(pady=5)
        
        # --- CONTROL GRID ---
        self.frame_controls = tk.Frame(self.frame, padx=20, pady=20)
        ...
        # Log
        self.lbl_log = tk.Label(self.frame, text="Log: Ready", fg="gray", font=("Courier", 10))
    ...
    def run_map_sequence(self, key):
        ...
        self.root_window.after(0, lambda: self.update_ui_after_map(key))
...
# --- MAIN SESSION MASTER (Pipeline 1) ---
class SessionMasterV19:
    def __init__(self, root):
        self.root = root
        self.root.title("Roulette Master V19 (Integrated)")
        ...
        # TAB 1: Session History Setup (Renamed as requested)
        self.tab_tech = tk.Frame(self.notebook, bg="#f0f0f0")
        self.notebook.add(self.tab_tech, text="Session History Setup")

        # Container for technical controls
        self.frame_tech_controls = tk.Frame(self.tab_tech, bg="#f0f0f0", pady=10)
        self.frame_tech_controls.pack(fill="x")

        self.btn_calib = tk.Button(self.frame_tech_controls, text="Adjust Grid", command=self.open_calibration)
        self.btn_calib.pack(side="left", padx=10)
        
        self.btn_test_seq = tk.Button(self.frame_tech_controls, text="TEST 0-36 LOOP", bg="orange", fg="black", font=("Arial", 9, "bold"), command=self.toggle_sequence_test)
        self.btn_test_seq.pack(side="left", padx=10)

        self.btn_matrix = tk.Button(self.frame_tech_controls, text="MATRIX REPORT", bg="#663399", fg="white", font=("Arial", 9, "bold"), command=self.open_matrix)
        self.btn_matrix.pack(side="left", padx=10)

        self.btn_float = tk.Button(self.frame_tech_controls, text="FLOAT", bg="#007bff", fg="white", font=("Arial", 9, "bold"), command=self.toggle_float)
        self.btn_float.pack(side="left", padx=10)

        tk.Label(self.frame_tech_controls, text="Opacity:", bg="#f0f0f0").pack(side="left")
        self.slider_alpha = tk.Scale(self.frame_tech_controls, from_=0.3, to=1.0, resolution=0.1, orient="horizontal", command=self.set_alpha, bg="#f0f0f0", bd=0)
        self.slider_alpha.set(1.0)
        self.slider_alpha.pack(side="left", padx=5)

        # TAB 2: Chip Calibration (NEW INTEGRATION)
        self.tab_chips = tk.Frame(self.notebook, bg="#f0f0f0")
        self.notebook.add(self.tab_chips, text="Chip Calibration")
        
        # Initialize the Step 2 Manager inside this tab
        self.chip_manager = ChipCalibrationManager(self.tab_chips, self.root)

        # TAB 3: Session / Monitor
        self.tab_session = tk.Frame(self.notebook, bg="white")
        self.notebook.add(self.tab_session, text="Session / Monitor")

        # Container for session controls
        self.frame_session_controls = tk.Frame(self.tab_session, bg="white", pady=10)
        self.frame_session_controls.pack(fill="x")

        self.btn_start = tk.Button(self.frame_session_controls, text="START MONITOR", bg="green", fg="white", font=("Arial", 12, "bold"), command=self.handle_start_stop)
        self.btn_start.pack()

        self.lbl_status = tk.Label(self.tab_session, text="System: Ready", font=("Arial", 10, "bold"), fg="gray", bg="white", relief="sunken")
        self.lbl_status.pack(fill="x", padx=5, pady=5)
        
        self.frame_results = tk.Frame(self.tab_session, bg="white")
